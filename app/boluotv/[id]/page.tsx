...
export default function ProjectDetail() {
  ...
  const finalizeOutcome = async (result: 'cooperated' | 'not_cooperated') => {
    ...
    const ok = window.confirm(result === 'cooperated'
      ? '确认已达成合作？确认后仍可继续沟通后续制作，项目不再重新招募。'
  ...
  // 任务完成(cooperated → closed)
  const completeProject = async () => {
    if (!myReservation) return;
    if (!window.confirm('确认任务已完成？项目将标记为已完成结束。')) return;
    try {
      const sb = createClient();
      const { data: { session } } = await sb!.auth.getSession();
      if (!session) { window.location.href = '/auth'; return; }
      const res = await fetch(`/api/commissions/${id}/complete`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${session.access_token}` },
        body: JSON.stringify({ action: 'complete', reservationId: myReservation.id }),
      });
      const data = await res.json();
      if (!res.ok) { alert(data.error || '操作失败'); return; }
      await load();
  ...
  return (
    <Shell>
      <div className={`${inExclusive ? 'max-w-[1400px]' : 'max-w-[1100px]'} mx-auto px-6 py-10`}>
        ...
        {inExclusive ? (
          ...
          <div className="mt-5 grid grid-cols-1 lg:grid-cols-2 gap-6 items-start">
            ...
            <div className="flex flex-col gap-8">
              {projectHead}
              {isOwner && applicantList}
            </div>
            {/* 右列: 独家沟通块 + 聊天 */}
            <div className="flex flex-col gap-5">
              {isOwner ? ownerExclusive : creatorExclusive}
          ...
          /* 非独家沟通状态: 单列全宽 */
          <>
            <div className="mt-5">{projectHead}</div>

            {isOwner ? (
              <div className="mt-8">{applicantList}</div>
            ) : myApplication ? (
