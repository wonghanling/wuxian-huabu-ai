          ...
          >
            <div className="relative" style={{ maxWidth: '70vw', maxHeight: '70vh' }} onClick={(e) => e.stopPropagation()}>
              {lightboxVideo.includes('.mp4') || lightboxVideo.includes('video') ? (
                ...
                <img src={lightboxVideo} alt="大图" className="rounded-xl object-contain" style={{ maxWidth: '70vw', maxHeight: '70vh', imageRendering: '-webkit-optimize-contrast', transform: 'translateZ(0)' }} />
              )}
              <button
...
Maintain strong visual consistency in every panel.`;
        ...
        {cardType === 'image' && (!isMinimized || isCollapsed) && showRefImagePanel && ['nano-banana', 'nano-banana-pro', 'nano-banana-pro-multi', 'gpt-image-2', 'gpt-image-2-all', 'doubao-seedream-4-5-251128', 'flux-kontext', 'mj_imagine'].includes(model || '') && (() => {
          ...
          const localCount = localImgs.length + localUrls.length + (singleImg ? 1 : 0);
          // 连接数先按模型真实上限裁剪，再按剩余额度显示
          const connAvailable = Math.max(0, totalMax - localCount);
          const connShown = connectedImageCardImages.slice(0, connAvailable);
          const connCount = connShown.length;
          const currentTotal = connCount + localCount;
        ...
        >
          ...
          {isMinimized ? (
            ...
            {/* 模型选择 - 文字/视频卡片保留在卡片内，图片卡片移到右侧浮层 */}
            {cardType !== 'character' && cardType !== 'kling' && cardType !== 'image' && (
              <div className="mb-2">
                <label className="text-gray-400 text-xs mb-1 block">Model</label>
                <select
                  className="w-full bg-black/30 border border-white/8 rounded-lg p-2 text-white text-xs focus:outline-none focus:border-white/15 focus:bg-black/40 transition-all"
                  value={model || (cardType === 'text' ? 'gpt-5.2' : 'veo3.1-fast-t2v')}
                  onClick={(e) => e.stopPropagation()}
                  onPointerDown={(e) => e.stopPropagation()}
                  onChange={(e) => {
                    editor.updateShape({
            ...
            {cardType === 'image' && (
              <button
                ...
                onPointerDown={(e) => e.stopPropagation()}
              >
                {showImageSettingsPanel ? '收起参数设置 ▲' : `展开参数设置 ▼  ${model || 'nano-banana-pro'} · ${aspectRatio || '1:1'}`}
              </button>
            )}
              ...
              <button
                ...
                onClick={(e) => {
                  ...
                  editor.updateShape({
                    id: shape.id,
                    type: 'custom-card' as any,
                    props: { ...shape.props, showImageOutput: !showImageOutput },
                  });
                }}
                onPointerDown={(e) => e.stopPropagation()}
              >
