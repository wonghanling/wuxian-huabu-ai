...
export class GemStep4CardUtil extends BaseBoxShapeUtil<GemStep4CardShape> {
  ...
  component(shape: GemStep4CardShape) {
    ...
    const hasActiveTask = !!(isGenerating || showSettingsPanel || showImageOutput || isMinimized);
    ...
    const generate = async () => {
      ...
      const progressTimer = setInterval(() => {
        ...
        if (!ls) { clearInterval(progressTimer); return; }
        editor.updateShape({ id: shape.id, type: 'gem-step4-card' as any, props: { ...ls.props, generationProgress: progress } });
      }, 3000);
      ...
      try {
        ...
        if (data.pending && data.requestId) {
          ...
          const poll = async (): Promise<void> => {
            attempts++;
            if (!editor.getShape(shape.id)) { clearInterval(progressTimer); return; }
