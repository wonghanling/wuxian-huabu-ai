...
import { findUsdTier } from '@/lib/usd-recharge';
...
export async function POST(req: NextRequest) {
  try {
    ...
    // 金额必须命中档位表 —— 不接受前端传任意值，否则可用 $0.01 换 ¥600
    ...
    // 先建单后下单:webhook 到达时靠 order_no 回查用户与应入账金额。
    // 入账金额只认库里的 amount_rmb，不认 Stripe 回传值。
    const { error: orderError } = await supabaseAdmin
      ...
      .insert({
        ...
        meta: { usd: tier.usd, currency: 'usd', rate: tier.cny / tier.usd },
      });
      ...
      line_items: [
        {
          price_data: {
            currency: 'usd',
            unit_amount: tier.usd * 100,   // Stripe 以最小货币单位计价
            product_data: {
              name: `Filmavo 余额充值 ¥${tier.cny}`,
              description: `支付后账户余额增加 ¥${tier.cny}`,
            },
          },
          quantity: 1,
        },
    ...
    await supabaseAdmin
      ...
      .update({
        trade_no: session.id,
        meta: { usd: tier.usd, currency: 'usd', rate: tier.cny / tier.usd, session_id: session.id },
      })
      .eq('order_no', orderId);
