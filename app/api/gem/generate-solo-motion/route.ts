...
const SYSTEM_3X3 = `You are a strict structured video prompt generator. Return VALID JSON ONLY. Do not output markdown, explanation, comments, or any extra text. The uploaded image is a 3x3 storyboard containing exactly 9 visual moments. Generate exactly 9 shot objects in visual order from left to right, top to bottom. The image is the primary source of truth. user_direction is only a secondary guide and must not override visible image content. The output JSON MUST contain ONLY two top-level keys: shots and global_constraints. The shots array MUST contain exactly 9 objects. Each shot object MUST contain exactly these keys: shot, transition_type, motion_intent, duration_control, keep_static, camera_control, final_video_prompt. No field may be empty. shot must be the number 1 to 9 in order. transition_type must be either morph_action or cut. motion_intent must be 8 to 20 English words and only describe the visible motion or change in that shot. duration_control must be slow, normal, or fast. keep_static must be an array of elements that should remain unchanged. camera_control must contain exactly two keys: movement and intensity. camera_control.movement must be one of: static, zoom_in, zoom_out, pan_left, pan_right, follow. camera_control.intensity must be one of: subtle, normal, dramatic. final_video_prompt must be written in this order: camera movement, subject action, timing, narrative or motion intent, constraints. Do NOT skip any shot. Do NOT merge shots. Do NOT summarize multiple shots into one. Do not invent new scenes, characters, objects, or events not supported by the image. Follow visible continuity. If scene change exists follow it. If no scene change do NOT add one. Do not describe frame numbers. The final top-level key MUST be global_constraints, and its value MUST be exactly: no grid, no panels, no borders, no collage layout, maintain scene continuity, follow visible continuity, if scene change exists follow it, if no scene change do not add one, do not describe frame numbers. Do NOT modify, shorten, translate, reformat, or omit this string. If the output is not valid JSON, if any required key is missing, if any field is empty, if the shots array does not contain exactly 9 objects, or if global_constraints is missing or changed, the output is invalid.`;
...
{"shots":[{"shot":1,"transition_type":"","motion_intent":"","duration_control":"","keep_static":[],"camera_control":{"movement":"","intensity":""},"final_video_prompt":""},{"shot":2,"transition_type":"","motion_intent":"","duration_control":"","keep_static":[],"camera_control":{"movement":"","intensity":""},"final_video_prompt":""},{"shot":3,"transition_type":"","motion_intent":"","duration_control":"","keep_static":[],"camera_control":{"movement":"","intensity":""},"final_video_prompt":""},{"shot":4,"transition_type":"","motion_intent":"","duration_control":"","keep_static":[],"camera_control":{"movement":"","intensity":""},"final_video_prompt":""},{"shot":5,"transition_type":"","motion_intent":"","duration_control":"","keep_static":[],"camera_control":{"movement":"","intensity":""},"final_video_prompt":""},{"shot":6,"transition_type":"","motion_intent":"","duration_control":"","keep_static":[],"camera_control":{"movement":"","intensity":""},"final_video_prompt":""},{"shot":7,"transition_type":"","motion_intent":"","duration_control":"","keep_static":[],"camera_control":{"movement":"","intensity":""},"final_video_prompt":""},{"shot":8,"transition_type":"","motion_intent":"","duration_control":"","keep_static":[],"camera_control":{"movement":"","intensity":""},"final_video_prompt":""},{"shot":9,"transition_type":"","motion_intent":"","duration_control":"","keep_static":[],"camera_control":{"movement":"","intensity":""},"final_video_prompt":""}],"global_constraints":"no grid, no panels, no borders, no collage layout, maintain scene continuity, follow visible continuity, if scene change exists follow it, if no scene change do not add one, do not describe frame numbers."}
      ...
      if (jsonMatch) {
        ...
        if (parsed.shots) {
          finalPrompt = parsed.shots
            .map((s: any) => {
              // 新 3x3 schema 用 final_video_prompt，旧 schema 用 camera/action
              if (s.final_video_prompt) {
                return `Shot ${s.shot}: ${s.final_video_prompt}`;
              }
